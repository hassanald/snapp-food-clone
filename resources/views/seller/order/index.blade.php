                            ...
                            <td class="px-6 py-4 flex justify-center gap-2">
                                <form method="post" action="{{route('seller.order.status' , $order->id)}}">
                                    ...
                                    @switch($order->status->title)
                                        @case('pending')
                                            <input type="hidden" name="status_id" value="{{\App\Models\OrderStatus::getPreparingId()}}">
                                            <button class="bg-yellow-500 text-white px-3 py-2 rounded-lg">Preparing</button>
                                        @break
                                        @case('preparing')
                                            <input type="hidden" name="status_id" value="{{\App\Models\OrderStatus::getDeliveringId()}}">
                                            <button class="bg-yellow-500 text-white px-3 py-2 rounded-lg">Delivering</button>
                                        @break
                                        @case('delivering')
                                            <input type="hidden" name="status_id" value="{{\App\Models\OrderStatus::getDeliveredId()}}">
                                            <button class="bg-yellow-500 text-white px-3 py-2 rounded-lg">Delivered</button>
                                        @break
                                        @default
                                            <input type="hidden" name="status_id" value="{{\App\Models\OrderStatus::getPreparingId()}}">
                                            <button class="bg-red-500 text-white px-3 py-2 rounded-lg">Preparing</button>
                                    @endswitch
                                </form>
                                <form method="post" action="{{route('seller.order.status' , $order->id)}}">
                                    @method('put')
                                    @csrf
                                    <input type="hidden" name="status_id" value="{{\App\Models\OrderStatus::getRejectId()}}">
                                    <button class="bg-red-500 text-white px-3 py-2 rounded-lg">Reject</button>
                                </form>
                            </td>
                        </tr>
                    @endforeach
