<x-seller>
    <div class="grid w-full gap-6">
        ...
        <div class="bg-gray-200 shadow-xl w-full p-6 rounded-lg">
            ...
            {{--Form--}}
            <form method="post" class="w-3/5 mx-auto" action="{{route('seller.rest.store')}}">
                @csrf
                <div class="mt-6">
                ...
                <div class="mt-6">
                    ...
                    <x-text-input id="acc_number" class="block mt-1 w-full" type="text" name="acc_number" :value="old('acc_number')" autofocus />
                ...
                <div class="mt-6">
                    ...
                    <select class="border-gray-300 mt-1 focus:border-pink-500 focus:ring-pink-500 rounded-md shadow-sm" name="restaurant_category_id" id="restaurant_category_id">
                        ...
                        @foreach($categories as $category)
                            <option value="{{$category->id}}" <?= (old('restaurant_category_id') == $category->id) ? 'selected' : '' ?>>{{$category->title}}</option>
                        @endforeach
                    </select>
