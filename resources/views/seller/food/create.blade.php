<x-seller>
    <div class="grid w-full gap-6">
        ...
        <div class="bg-gray-200 shadow-xl w-full p-6 rounded-lg">
            <h2 class="text-xl font-semibold text-center">Create New Food</h2>
            {{--Form--}}
            <form method="post" class="w-3/5 mx-auto" action="{{route('seller.food.store')}}">
                @csrf
                <div class="mt-6">
                ...
                <div class="mt-6">
                    ...
                    <x-text-input id="raw_materials" class="block mt-1 w-full" type="text" name="raw_materials" :value="old('raw_materials')" autofocus />
                    <x-input-error :messages="$errors->get('raw_materials')" class="mt-2" />
                </div>

                <div class="mt-6">
                    <x-input-label for="price" :value="__('Price')" />
                    <x-text-input id="price" class="block mt-1 w-2/5" type="text" name="price" :value="old('price')" autofocus />
                ...
                <label id="toggle" style="display: none;" class="mt-4 relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="is_party" value="1" class="sr-only peer">
                ...
                <div class="mt-6">
                    ...
                    <select class="border-gray-300 mt-1 focus:border-pink-500 focus:ring-pink-500 rounded-md shadow-sm" name="food_category_id" id="food_category_id">
                        ...
                        @foreach($categories as $category)
                            <option value="{{$category->id}}" <?= (old('food_category_id') == $category->id) ? 'selected' : '' ?>>{{$category->title}}</option>
                        @endforeach
                    </select>
